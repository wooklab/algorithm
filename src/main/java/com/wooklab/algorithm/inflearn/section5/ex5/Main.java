package com.wooklab.algorithm.inflearn.section5.ex5;
 ...
 * 아래 그림(강의참고)은 위 조건을 만족하는 예를 보여준다. 수평으로 그려진 굵은 실선은 쇠막대기이고, 점은 레이저의 위치,
...
public class Main {
    ...
    private int mySolution(String input) {
        int answer = 0;
        Stack<Character> stack = new Stack<>();
        int preStackSize = -1;
        ...
        for (char c : input.toCharArray()) {
            if (c == '(') {
                ...
                stack.pop();
                if (!stack.isEmpty() && stack.size() == preStackSize - 1 && preOpen) {
                    answer += stack.size();
                } else if (!stack.isEmpty() || !preOpen) {
                    answer++;
                }
                preOpen = false;
            }
